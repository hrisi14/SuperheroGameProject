...
{
	outFile.write((const char*)&myStr._length, sizeof(myStr._length));
	outFile.write(myStr._data, myStr._length);
	return outFile;
}

std::istream& operator<<(MyString& myStr, std::istream& inFile)
{
	inFile.read((char*)&myStr._length, sizeof(myStr._length));
	myStr._data = new char[myStr._length + 1];
	inFile.read(myStr._data, myStr._length);
	myStr._data[myStr._length] = '\0';
	return inFile;
}